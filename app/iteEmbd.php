...
try { // O que deve ser feito.
 ...
 $comandoSQLItemNota->execute(array(
   ...
   ":codBar"          => $codBar 
      ...
      $calculoPrecoMedio = ("SELECT round(COALESCE((COALESCE(sum(c.vl_total_ite),0) /  ( COALESCE(sum(c.qtd_ite),0)) ),0) , 2) as vl_unit_ite
                             FROM c_item_em c
...
//atualiza valor total medio kardex

   $idMovItemKardex = ("SELECT id_item_em as id_item_em
                             FROM c_item_em c
                             WHERE c.lote_prod_ite_nf like '%".$loteMaterial."%'
                             GROUP BY
                             c.lote_prod_ite_nf") ;
    ...
    $comandoSQLAtualizaKardex = $conexao->prepare(
       ...
       "UPDATE c_kardex 
                      SET
                        saldoVlKard = ( $precoMed[vl_unit_ite] * (SELECT 
                                                                     COALESCE 
                                                                          (COALESCE(sum(ent.qtd_ite ),0)  -
                                                                          COALESCE(totalQtdSaida.total_saida,0) ,0)   as saldoQtdKard
                                                                          FROM c_item_em ent
                                                                          LEFT JOIN (
                                                                                      SELECT
                                                                                         sai.lote_mat_saida,
                                                                                         COALESCE(SUM(sai.qtd_saida_mat),0) total_saida
                                                                                      FROM c_saida_material sai 
                                                                                      WHERE
                                                                                          sai.lote_mat_saida = '{$loteMaterial}'
                                                                                      GROUP BY
                                                                                      sai.lote_mat_saida 
                                                                                   
                                                                                    ) AS totalQtdSaida on totalQtdSaida.lote_mat_saida = ent.lote_prod_ite_nf
                                                                          WHERE
                                                                              ent.lote_prod_ite_nf = '{$loteMaterial}'
                                                                          GROUP BY
                                                                               ent.lote_prod_ite_nf )  ) ,
                          vlPreMedKard =  $precoMed[vl_unit_ite]                 
                       
                       WHERE loteKard  = '{$loteMaterial}'  AND idMovItemKard = (SELECT   id_item_em AS id_item_em
                                                                                   FROM c_item_em c
                                                                                   WHERE c.lote_prod_ite_nf = '{$loteMaterial}'
                                                                                 
                                                                                   ORDER BY C.id_item_em DESC
                                                                                   LIMIT 1);"
   ...
   $comandoSQLAtualizaKardex->execute(array( ));
